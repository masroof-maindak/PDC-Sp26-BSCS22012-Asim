...
def test_health_check():
    ...
    try:
        ...
        if verify_student_id_header(response):
            print_success(f"X-Student-ID header is present: {response.headers['X-Student-ID']}")
        else:
...
def test_normal_operation():
    ...
    try:
        ...
        if verify_student_id_header(response):
            print_success(f"X-Student-ID header present: {response.headers['X-Student-ID']}")
        else:
...
def test_circuit_recovery():
    ...
    try:
        ...
        response = httpx.post(
            f"{BASE_URL}/generate-text",
            params={"prompt": "Are you back online?"}
        )
...
def main():
    ...
    print(f"{Fore.CYAN}This script validates the Circuit Breaker implementation{Style.RESET_ALL}")
    ...
    if passed == total:
        print(f"{Fore.GREEN}✓ All tests passed! Circuit Breaker is working correctly.{Style.RESET_ALL}\n")
    else:
        print(f"{Fore.RED}✗ Some tests failed. Please check the implementation.{Style.RESET_ALL}\n")
